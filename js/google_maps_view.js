...
var glocation = (function (index,
    ...
    this.compileHtmlAddress = function() {
        ...
        if (this.address_line_3) {
            address += "<br /><br />"+this.address_line_3;
        }
        if (this.address_line_4) {
            address += "<br />"+this.address_line_4;
        }
        return address;
    };
...
var map_markers_attrs = (function (map_id, markers_array) {
    this.map_id = map_id;
    this.markers_array = JSON.parse('['+markers_array+']');
});
    ...
    if (map_markers_attrs_array.length) {
        ...
        for (var i=0; i<map_markers_attrs_array.length; i++) {
            ...
            if (document.getElementById("maps_canvas_"+unique_map_id)) {
                ...
                maps[unique_map_id] = new google.maps.Map(
                    document.getElementById("maps_canvas_"+unique_map_id), mapOptions
                );
                ...
                if (markers_array.length) {
                    ...
                    var zoom_level = 7;
                    var map_center = new google.maps.LatLng(8.47, -79.52);
                    show_map = false;
                }
...
function placeMarker(glocation, unique_map_id) {
    var customIcon = new google.maps.MarkerImage(
        global_map_icons_path+'icon_map_large_w_shadow.png',
...
function showInfoWindow(glocation, marker, map) {
    ...
    if ((pathname.indexOf('/listings/') !== -1 || pathname.indexOf('/articles/') !== -1) &&
        hash.indexOf('#/ses/') === -1 &&
        hash.indexOf('#/sen/') === -1) {
        isListingPage = true;
    }
    ...
    if (glocation.listing_logo) {
        ...
        windowHtml += '<img width="70px" style="float: left; padding-right: 10px;" src="'+
            global_server_path+'/users_images/logos/'+glocation.listing_logo+'" />';
        if (glocation.listing_seo_title) {
            windowHtml += '</a>';
        }
    }
    if (glocation.listing_seo_title && !isListingPage) {
        windowHtml += '<a href="#listing_id-' + glocation.listing_seo_title + '">';
        windowHtml += '<strong>'+glocation.listing_title+'</strong></a><br />';
    }
    else {
        windowHtml += '<strong>'+glocation.listing_title+'</strong><br />';
    }
    ...
    if (glocation.listing_seo_title && !isListingPage) {
        windowHtml += '<strong><a href="#listing_id-'+glocation.listing_seo_title + '">'+
            view_summary_label+'</a></strong>&nbsp;&nbsp;&nbsp;';
    }
    if (glocation.listing_url && !isListingPage) {
        windowHtml += '<strong><a href="' + lang_seo_full + glocation.listing_url + '">' + view_listing_label + '</a></strong>';
    }
    if (glocation.listing_url || glocation.listing_seo_title) {
